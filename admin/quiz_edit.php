...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($title) && !empty($description)) {
        $stmt = $conn->prepare("UPDATE quizzes SET title = ?, description = ?, time_limit = ?, passing_score = ?, status = ?, updated_at = NOW() WHERE id = ?");
        $stmt->bind_param("ssiisi", $title, $description, $time_limit, $passing_score, $status, $quiz_id);
        
        if ($stmt->execute()) {
            header("Location: quizzes.php?updated=1");
            exit();
        } else {
            $message = '<div class="alert alert-danger">Failed to update quiz!</div>';
        }
    } else {
        ...
        <div class="row">
            ...
            <main class="col-md-10 ms-sm-auto px-md-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3">
                    <h1 class="h2"><i class="fas fa-edit me-2"></i>Edit Quiz</h1>
                    <div>
                        <a href="quiz_questions.php?id=<?php echo $quiz_id; ?>" class="btn btn-info me-2">
                ...
                <div class="card">
                    <div class="card-header">
                        <h5><i class="fas fa-question-circle me-2"></i>Edit Quiz: <?php echo htmlspecialchars($quiz['title']); ?></h5>
                    </div>
                    <div class="card-body">
                        <form method="POST">
                            <div class="row">
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" class="form-control" id="time_limit" name="time_limit" value="<?php echo $quiz['time_limit']; ?>" min="1" max="180">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        <label for="passing_score" class="form-label">Passing Score (%)</label>
                                        <input type="number" class="form-control" id="passing_score" name="passing_score" value="<?php echo $quiz['passing_score']; ?>" min="1" max="100">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control" id="status" name="status" required>
                                            ...
                                            <option value="rejected" <?php echo $quiz['status'] == 'rejected' ? 'selected' : ''; ?>>Rejected</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <p class="text-muted">
                                        <strong>Created:</strong> <?php echo date('Y-m-d H:i', strtotime($quiz['created_at'])); ?>
                                    </p>
                                </div>
                            </div>
